...
def plot_results(X: pd.DataFrame, size=3, category_col: str = "cluster") -> None:
    ...
    if size == 2:
        ...
        fig.update_traces(marker=dict(size=3))
        fig.update_layout(title="PCA 2D", template="plotly_white", font=dict(size=14))
        ...
        fig.update_traces(marker=dict(size=3))
        fig.update_layout(title="PCA 3D", template="plotly_white", font=dict(size=14))
